...
export const downloadAllPayslipsPDF = async (req, res) => {
  try {
    ...
    // Get bank details from Salary collection
    const userIds = payrolls.map(p => p.userId._id);
    const salaries = await Salary.find({ userId: { $in: userIds } });
    const salaryMap = {};
    salaries.forEach(salary => {
    ...
    // Create PDF document in LANDSCAPE mode to accommodate extra columns
    const doc = new PDFDocument({
      margin: 40,
      layout: 'landscape',   // gives ~792x612 (width x height)
      size: 'A4'             // or 'LETTER' – landscape flips width/height
    });
    ...
    // --- Table Configuration (8 columns) ---
    const tableTop = doc.y;
    const tableLeft = 40;
    const rowHeight = 26;
    const colWidths = [100, 80, 70, 80, 80, 70, 70, 70]; // total = 540 (fits landscape ~792)
    const headers = [
      'Employee Name',
      'Department',
      'Net Salary (₹)',
    ...
    payrolls.forEach((payroll, index) => {
     ...
     let accountNumber = bank.accountNumber || '';
      ...
      rowData.forEach((cell, i) => {
        doc.text(cell, xPosRow, yPos, {
          width: colWidths[i],
          align: i === 8 ? 'right' : 'left', // right-align net salary
          lineBreak: false,
          ellipsis: true
        });
        xPosRow += colWidths[i];
      ...
      if (yPos > doc.page.height - 70) {
        ...
        doc.font('Helvetica-Bold').fontSize(9);
        let xHeader = tableLeft;
        headers.forEach((header, i) => {
          doc.text(header, xHeader, yPos - rowHeight, { width: colWidths[i], align: 'left' });
          xHeader += colWidths[i];
        });
        doc.moveTo(tableLeft, yPos - rowHeight + 15)
           .lineTo(tableLeft + colWidths.reduce((a, b) => a + b, 0), yPos - rowHeight + 15)
           .stroke();
      }
    });
...
export const downloadDetailedPayslip = async (req, res) => {
  try {
    ...
    const payroll = await Payroll.findById(req.params.id)
      .populate("userId", "name email employeeId department company shifts")
      .populate("generatedBy", "name email");
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=payslip_${payroll.userId.employeeId || payroll.userId._id}_${payroll.month}_${payroll.year}.pdf`
    );
    ...
    doc.fontSize(24)
       .font('Helvetica-Bold')
       .fillColor('#1e3c72')  // Dark blue color for company name
       .text(payroll.userId.company?.toUpperCase() || 'COMPANY NAME', { 
         align: 'center',
    ...
    //    .font('Helvetica')
    //    .fillColor('#333333')
    //    .text('123 Business Park, Corporate Zone', { 
    //      align: 'center',
    //      width: doc.page.width - 100 
    //    })
    //    .text('City - 123456, State, Country', { 
    //      align: 'center',
    //      width: doc.page.width - 100 
    //    })
    //    .text('GST: 1234567890 | PAN: ABCDE1234F', { 
    //      align: 'center',
    //      width: doc.page.width - 100 
    //    })
    //    .fillColor('black')
    ...
    // Horizontal line with company color
    doc.moveTo(50, doc.y)
    ...
    doc.fontSize(12)
       .font('Helvetica')
       .fillColor('#333333')
       .text(`FOR THE PERIOD: ${payroll.month} ${payroll.year}`, { 
         align: 'center',
         width: doc.page.width - 100 
       })
       .fillColor('black')
    ...
    // ========== EMPLOYEE & BANK DETAILS (TWO COLUMN LAYOUT) ==========
    const startY = doc.y;
    const leftColumnX = 50;
    const rightColumnX = 320; // Adjusted for better spacing
    ...
    const empBoxY = drawSectionTitle('EMPLOYEE INFORMATION', leftColumnX, startY);
    
    // Draw light background for employee details
    doc.rect(leftColumnX - 5, empBoxY - 8, 250, 95)
       .fillOpacity(0.05)
       .fill('#1e3c72')
       .fillOpacity(1);
    
    let currentY = empBoxY;
    ...
    currentY = drawInfoRow('Name', payroll.userId.name || 'N/A', leftColumnX + 5, empBoxY);
    currentY = drawInfoRow('Employee ID', payroll.userId.employeeId || 'N/A', leftColumnX + 5, currentY);
    currentY = drawInfoRow('Department', payroll.userId.department || 'N/A', leftColumnX + 5, currentY);
    currentY = drawInfoRow('Shift', payroll.userId.shifts || 'N/A', leftColumnX + 5, currentY);
    currentY = drawInfoRow('Email', payroll.userId.email || 'N/A', leftColumnX + 5, currentY);

    // Draw Bank Details Box
    drawSectionTitle('BANK DETAILS', rightColumnX, startY);
    
    // Draw light background for bank details
    doc.rect(rightColumnX - 5, empBoxY - 8, 250, 95)
       .fillOpacity(0.05)
       .fill('#1e3c72')
       .fillOpacity(1);
    ...
    if (salary?.bankAccount) {
      currentY = drawInfoRow('Bank Name', salary.bankAccount.bankName || 'N/A', rightColumnX + 5, currentY);
      currentY = drawInfoRow('Account No', salary.bankAccount.accountNumber || 'N/A', rightColumnX + 5, currentY);
      currentY = drawInfoRow('IFSC Code', salary.bankAccount.ifscCode || 'N/A', rightColumnX + 5, currentY);
      currentY = drawInfoRow('Branch', salary.bankAccount.branch || 'N/A', rightColumnX + 5, currentY);
      
      if (salary.bankAccount.accountType) {
        currentY = drawInfoRow('Account Type', salary.bankAccount.accountType, rightColumnX + 5, currentY);
      }
    } else {
      doc.fontSize(10)
         .font('Helvetica-Italic')
         .fillColor('#999999')
         .text('Bank details not available', rightColumnX + 5, currentY);
    }
    
    doc.fillColor('black');
    doc.y = Math.max(startY + 140, currentY);
    doc.moveDown(1.5);

    // ========== ATTENDANCE SUMMARY (TWO COLUMN) ==========
    doc.fontSize(14)
       .font('Helvetica-Bold')
       .fillColor('#1e3c72')
       .text('ATTENDANCE SUMMARY', { 
         underline: true,
         underlineColor: '#1e3c72',
         underlineThickness: 1.5
       });
    
    doc.moveDown(0.8);
    
    const attendanceLeftY = doc.y;
    
    const attendanceLeft = [
      ['Present Days', payroll.presentDays || 0],
      ['Half Days', payroll.halfDays || 0],
      ['Leave Days', payroll.leaveDays || 0],
      ['Holiday Days', payroll.holidayDays || 0],
    ];

    const attendanceRight = [
      ['Absent Days', payroll.absentDays || 0],
      ['Overtime Hours', payroll.overtimeHours || 0],
      ['Total Working Days', payroll.totalWorkingDays || 0],
      ['Total Paid Days', payroll.totalPaidDays || 0],
    ];

    // Draw attendance in organized columns
    const drawAttendanceColumn = (items, x, startY) => {
      items.forEach(([label, value], index) => {
        doc.fontSize(10)
           .font('Helvetica')
           .fillColor('#333333')
           .text(label, x, startY + (index * 18), { 
             continued: true,
             width: 100
           })
           .font('Helvetica-Bold')
    ...
    drawAttendanceColumn(attendanceLeft, leftColumnX, attendanceLeftY);
    drawAttendanceColumn(attendanceRight, rightColumnX, attendanceLeftY);
    
    doc.fillColor('black');
    doc.y = attendanceLeftY + (attendanceLeft.length * 18) + 15;
    doc.moveDown(1);
    ...
    const salaryStartY = doc.y;

    // Earnings Section (Left) with light background
    doc.rect(leftColumnX - 5, salaryStartY - 5, 250, 135)
       .fillOpacity(0.03)
       .fill('#1e3c72')
       .fillOpacity(1);
    ...
    doc.fontSize(12)
       .font('Helvetica-Bold')
       .fillColor('#1e3c72')
       .text('EARNINGS', leftColumnX, salaryStartY);
    doc.fillColor('black');
    
    const earnings = [
      ['Basic Salary', payroll.basicSalary || 0],
      ['Present Salary', payroll.presentSalary || 0],
      ['Half Day Salary', payroll.halfDaySalary || 0],
      ['Holiday Salary', payroll.holidaySalary || 0],
      ['Leave Salary', payroll.leaveSalary || 0],
      ['Overtime Salary', payroll.overtimeSalary || 0],
    ...
    earnings.forEach(([label, value], index) => {
      doc.fontSize(10)
         .font('Helvetica')
         .fillColor('#333333')
         .text(label, leftColumnX, salaryStartY + 18 + (index * 16), { 
           continued: true,
           width: 120
         })
         .font('Helvetica-Bold')
    ...
    // Deductions Section (Right) with light background
    doc.rect(rightColumnX - 5, salaryStartY - 5, 250, 95)
       .fillOpacity(0.03)
       .fill('#1e3c72')
       .fillOpacity(1);
    ...
    doc.fontSize(12)
       .font('Helvetica-Bold')
       .fillColor('#1e3c72')
       .text('DEDUCTIONS', rightColumnX, salaryStartY);
    doc.fillColor('black');
    
    const deductions = [
      ['Absent Deductions', payroll.absentDeductions || 0],
      ['Salary Deductions', payroll.salaryDeductions || 0],
      ['Advance Payment', payroll.advancedPaymentDeduction || 0],
    ];
    ...
    deductions.forEach(([label, value], index) => {
      doc.fontSize(10)
         .font('Helvetica')
         .fillColor('#333333')
         .text(label, rightColumnX, salaryStartY + 18 + (index * 16), { 
           continued: true,
           width: 120
         })
         .font('Helvetica-Bold')
    ...
    doc.fillColor('black');
    doc.y = salaryStartY + (earnings.length * 16) + 25;
    doc.moveDown(1.5);
    ...
    // Create a highlighted box for payment summary
    const summaryBoxY = doc.y;
    doc.rect(45, summaryBoxY - 8, doc.page.width - 90, 90)
       .fillOpacity(0.08)
       .fill('#1e3c72')
       .fillOpacity(1);
    
    doc.strokeColor('#1e3c72')
       .lineWidth(1.5)
       .rect(45, summaryBoxY - 8, doc.page.width - 90, 90)
       .stroke()
       .strokeColor('black');

    // Summary content with better alignment
    doc.fontSize(8)
       .font('Helvetica')
       .fillColor('#333333')
       .text('Gross Salary:', leftColumnX + 15, summaryBoxY + 10, { 
         continued: true,
         width: 100
       })
       .font('Helvetica-Bold')
       .fillColor('#1e3c72')
       .text(formatCurrency(payroll.grossSalary || 0), { 
         width: 100,
         align: 'right'
       });
    ...
    doc.fontSize(8)
       .font('Helvetica')
       .fillColor('#333333')
       .text('Total Deductions:', leftColumnX + 15, doc.y + 20, { 
         continued: true,
         width: 100
       })
       .font('Helvetica-Bold')
       .fillColor('#dc3545')  // Red for deductions
       .text(formatCurrency(payroll.totalDeductions || 0), { 
         width: 100,
         align: 'right'
       });

    // NET Salary in right column with highlight
    doc.fontSize(8)
       .font('Helvetica-Bold')
       .fillColor('#1e3c72')
       .text('NET SALARY:', rightColumnX, summaryBoxY + 10, { 
         continued: true,
         width: 100
       })
       .font('Helvetica-Bold')
       .fillColor('#28a745')  // Green for net salary
       .text(formatCurrency(payroll.netSalary || 0), { 
         width: 120,
         align: 'right'
       });
    ...
    doc.fontSize(8)
       .font('Helvetica')
       .fillColor('#333333')
       .text('Payment Status:', rightColumnX, summaryBoxY + 45, { 
         continued: true,
         width: 100
       })
       .font('Helvetica-Bold')
       .fillColor(payroll.paymentStatus === 'paid' ? '#28a745' : '#ffc107')
       .text(payroll.paymentStatus?.toUpperCase() || 'PENDING', { 
         width: 120,
         align: 'right'
       });
    ...
    doc.fontSize(10)
       .font('Helvetica')
       .fillColor('#333333')
       .text(`Payment Method: ${salary?.paymentMethod || 'Not specified'}`, {
         continued: false,
         width: 200
       })
       .text(`Paid Date: ${formatDate(payroll.paidDate)}`, {
         continued: false,
         width: 200
       });
    ...
    //    .font('Helvetica')
    //    .fillColor('#666666')
    //    .text('Generated By:', 50, footerY, { 
    //      continued: true,
    //      width: 80
    //    })
    //    .font('Helvetica-Bold')
    //    .fillColor('#1e3c72')
    //    .text(payroll.generatedBy?.name || 'System', { 
    //      width: 150,
    //      align: 'left'
    //    });
    ...
    //    .font('Helvetica')
    //    .fillColor('#666666')
    //    .text('Generated On:', 50, footerY + 15, { 
    //      continued: true,
    //      width: 80
    //    })
    //    .font('Helvetica-Bold')
    //    .fillColor('#1e3c72')
    //    .text(formatDate(payroll.createdAt), { 
    //      width: 150,
    //      align: 'left'
    //    });
    ...
    //    .font('Helvetica')
    //    .fillColor('#666666')
    //    .text('For the Month:', rightColumnX, footerY, { 
    //      continued: true,
    //      width: 80
    //    })
    //    .font('Helvetica-Bold')
...
doc.font('Helvetica-Bold')
   .fillColor('#1e3c72')
   .text(payroll.generatedBy?.name || 'System', 130, footerY);
...
doc.font('Helvetica-Bold')
   .fillColor('#1e3c72')
   .text(formatDate(payroll.createdAt), 130, footerY + 15);
...
doc.font('Helvetica-Bold')
   .fillColor('#1e3c72')
   .text(`${payroll.month} ${payroll.year}`, rightColumnX + 80, footerY);
    ...
    doc.fontSize(8)
       ...
       .moveDown(0.3)
       .text(`© ${new Date().getFullYear()} ${payroll.userId.company || 'Company Name'}. All rights reserved.`, { 
         align: 'center',
         width: doc.page.width - 100
       });
